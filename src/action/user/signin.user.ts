...
export const signIn = async ({ password, email }: { password: string, email: string }): Promise<SignInResponse> => {
    try {
        connectToDB();
        ...
        // );
        return {
            message: "Sign in successful!", data: userData, token: createJWTToken(userData)
        };
    } catch (error: any) {
        console.log(error);
